...
# Function to validate if last digit of the plate matches the Pico y Placa
# restriction rules
def validate_plate_by_day(plate: str, day_of_week: str) -> bool:
    ...
    return (
        date_captured in pico_placa_restrictions
        and last_plate_digit in pico_placa_restrictions[date_captured]
    )
...
def validate_hour(time_to_string: str) -> bool:
    ...
    if (
        morning_start_schedule <= time_captured <= morning_end_schedule
        or afternoon_start_schedule <= time_captured <= afternoon_end_schedule
    ):
        return True
    return False
